...
const SALON_SERVICES = [
  { slug: 'hair', name: 'العناية بالشعر', desc: 'قص، صبغ، كيراتين، تمليس', color: 'from-pink-50 to-rose-50', border: 'border-pink-100' },
  { slug: 'skin', name: 'العناية بالبشرة', desc: 'تنظيف، تقشير، ماسكات', color: 'from-amber-50 to-yellow-50', border: 'border-amber-100' },
  { slug: 'nails', name: 'العناية بالأظافر', desc: 'مناكير، باديكير، جل', color: 'from-purple-50 to-violet-50', border: 'border-purple-100' },
  { slug: 'makeup', name: 'المكياج', desc: 'سهرة، عروس، يومي', color: 'from-red-50 to-pink-50', border: 'border-red-100' },
  { slug: 'spa', name: 'السبا والاسترخاء', desc: 'مساج، حمام مغربي، جاكوزي', color: 'from-teal-50 to-emerald-50', border: 'border-teal-100' },
  { slug: 'eyebrow', name: 'الحواجب والرموش', desc: 'ترتيب، صبغ، رموش', color: 'from-indigo-50 to-blue-50', border: 'border-indigo-100' },
]
    ...
    <main className="min-h-screen bg-white" dir="rtl">
      ...
      <section className="bg-rose-50 border-b border-rose-100 py-6">
        <div className="max-w-5xl mx-auto px-4 grid grid-cols-4 gap-4 text-center">
          {[
            { num: '+1000', label: 'صالون معتمد' },
            { num: '+50K', label: 'تقييم حقيقي' },
            { num: '+15', label: 'مدينة سعودية' },
            { num: '100%', label: 'مجاني للعميلة' },
          ].map(s => (
            ...
            {FEATURED_SALONS.map(salon => (
              <Link key={salon.slug} href={`/clinic/${salon.slug}`}
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="w-14 h-14 bg-rose-50 rounded-xl flex items-center justify-center text-xl font-bold text-rose-300">
                    {salon.name[0]}
                  </div>
      ...
      <section className="bg-rose-600 text-white py-16 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8 text-center">
            {[
              { title: 'ابحثي', desc: 'ابحثي عن الخدمة التي تريدينها في مدينتك' },
              { title: 'قارني', desc: 'قارني الأسعار والتقييمات بين أفضل الصالونات' },
              { title: 'احجزي', desc: 'احجزي موعدك بسهولة في الصالون المفضل' },
            ].map((s, i) => (
              <div key={i}>
                <div className="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center text-2xl font-bold mx-auto mb-4">{i + 1}</div>
      ...
      <footer className="bg-gray-900 text-white py-12 px-4">
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-8">
          <div>
            <div className="text-xl font-bold text-rose-400 mb-3">صالوني</div>
