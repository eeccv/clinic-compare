    ...
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            { label: 'إجمالي الصالونات', value: stats.total, icon: '💇‍♀️', color: 'bg-rose-50 text-rose-600' },
            { label: 'موثقة', value: stats.verified, icon: '✅', color: 'bg-green-50 text-green-600' },
            { label: 'مميزة', value: stats.featured, icon: '⭐', color: 'bg-amber-50 text-amber-600' },
            { label: 'حجوزات معلقة', value: stats.pending, icon: '📅', color: 'bg-blue-50 text-blue-600' },
          ].map(stat => (
            <div key={stat.label} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <div className={`text-3xl mb-3 w-12 h-12 rounded-xl flex items-center justify-center ${stat.color}`}>
                {stat.icon}
              </div>
        ...
        {loading ? (
              ...
              <div key={salon.id} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 bg-rose-50 rounded-xl flex items-center justify-center text-2xl flex-shrink-0">
                      💇‍♀️
                    </div>
                    <div>
                  ...
                  <div className="flex flex-wrap gap-2 flex-shrink-0">
                    <button onClick={() => updateSalon(salon.id, { verified: !salon.verified })}
                      className={`text-xs py-1.5 px-3 rounded-lg border transition-colors ${
                        salon.verified ? 'bg-green-50 border-green-200 text-green-600 hover:bg-green-100' : 'bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100'
                      }`}>
                      {salon.verified ? '✓ موثق' : 'توثيق'}
                    </button>
                    <button onClick={() => updateSalon(salon.id, { featured: !salon.featured })}
                      className={`text-xs py-1.5 px-3 rounded-lg border transition-colors ${
                        salon.featured ? 'bg-amber-50 border-amber-200 text-amber-600 hover:bg-amber-100' : 'bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100'
                      }`}>
          ...
          <div className="space-y-4">
            ...
            {bookings.map(b => (
              <div key={b.id} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
