...
const STATS = [
  { label: 'إجمالي المشاهدات', value: '2,847', change: '+12%', up: true },
  { label: 'التقييمات الجديدة', value: '23', change: '+5%', up: true },
  { label: 'طلبات التواصل', value: '156', change: '-3%', up: false },
...
const RECENT_REVIEWS = [
  ...
  { id: '3', author: 'نورا', rating: 5, comment: 'أفضل صالون في الرياض!', date: '2024-12-05' },
]
...
const NAV_ITEMS = [
  ...
  { label: 'التقييمات', href: '/dashboard/bookings' },
  { label: 'الإحصائيات', href: '/dashboard' },
  { label: 'الإعدادات', href: '/dashboard' },
]
      ...
      <div className="flex">
          ...
          <nav className="space-y-1">
            {NAV_ITEMS.map(item => (
              <Link key={item.label} href={item.href}
                ...
                {item.label === 'الحجوزات' && pendingBookings.length > 0 && (
        ...
        <main className="flex-1 mr-64 p-8">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/salon-dashboard" className="btn-primary text-sm">تعديل الملف الشخصي</Link>
          </div>
                  ...
                  <div key={b.id} className="flex items-center justify-between p-4 bg-amber-50 rounded-xl border border-amber-100">
                    <div className="space-y-1">
                      ...
                      <div className="text-sm text-gray-600">
                        {b.service?.name || 'خدمة'} | {new Date(b.date).toLocaleDateString('ar-SA')} | {b.time}
                      </div>
                      <div className="text-sm font-bold text-rose-600">{b.totalPrice} ريال</div>
                    </div>
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="col-span-2 bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              <div className="flex items-center justify-between mb-5">
                ...
                <Link href="/dashboard/bookings" className="text-sm text-rose-500 hover:underline">عرض الكل</Link>
              </div>
              <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                ...
                <div className="space-y-2">
                  {[
                    { label: 'إضافة خدمة جديدة', href: '/salon-dashboard' },
                    { label: 'تحديث الأسعار', href: '/salon-dashboard' },
                    { label: 'عرض الحجوزات', href: '/dashboard/bookings' },
                    { label: 'تعديل الصالون', href: '/salon-dashboard' },
                  ].map(action => (
